...
import { z } from "zod";
import type { BloodGlucoseReading, HealthDailyMetricRecord, HealthStepDayRecord, HealthWorkoutRecord } from "@family-os/shared";
...
function glucoseInLocalRange(
  rows: BloodGlucoseReading[],
  timezone: string,
  rangeStart: string,
  rangeEnd: string,
  limit: number
  ...
  return rows
    .filter((reading) =>
      isDateInInclusiveRange(localDateString(new Date(reading.measuredAt), timezone), rangeStart, rangeEnd)
    )
...
export function createBloodGlucoseRoutes(repository: HealthKitStore) {
  const routes = new Hono<{ Variables: AppVariables }>();
  routes.use("*", requireAuth());
  routes.get("/", zValidator("query", query), async (c) => {
    const parsed = c.req.valid("query");
    const window = await resolveHistoryWindow(repository, c.get("user").id, parsed);
    const { start, end } = expandedUtcRangeForLocalDays(window.rangeStart, window.rangeEnd);
    const rows = await repository.listHealthKitBloodGlucose(
      c.get("user").id,
      window.personId,
      start,
      end,
    ...
    return c.json({
      data: glucoseInLocalRange(rows, window.timezone, window.rangeStart, window.rangeEnd, parsed.limit)
    });
  });
  return routes;
}
